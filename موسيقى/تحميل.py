...
@Client.on_message(filters.command(["تحميل", "تنزيل","song"], prefixes=f"{HNDLR}"))
...
@Client.on_message(filters.command(["فيديو", "تحميل_فيديو","vid"], prefixes=f"{HNDLR}"))
async def vsong(client, message: Message):
    urlissed = get_text(message)

    pablo = await client.send_message(message.chat.id, f"**جار البحث عن** **{urlissed}**")
    if not urlissed:
